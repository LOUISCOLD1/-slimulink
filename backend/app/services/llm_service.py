...
def _build_prompt(question: str, lang: str = "zh") -> tuple[str, list[str]]:
    """
    ...
    # system prompt 只包含角色指令和政策文档，用户问题单独放在 user 消息中，防止 prompt 注入
...
def ask_policy_zhipu(question: str, lang: str = "zh") -> dict:
    ...
    if not ZHIPU_API_KEY:
        logger.error("未配置智谱API Key，请在.env文件中设置ZHIPU_API_KEY")
        return {"answer": "AI服务暂不可用，请联系管理员。建议拨打12345咨询。", "sources": []}

    prompt, sources = _build_prompt(question, lang)

    # 用户问题放在 user 角色中，系统指令放在 system 角色中，防止 prompt 注入
...
def ask_policy_deepseek(question: str, lang: str = "zh") -> dict:
    ...
    if not DEEPSEEK_API_KEY:
        logger.error("未配置DeepSeek API Key，请在.env文件中设置DEEPSEEK_API_KEY")
        return {"answer": "AI服务暂不可用，请联系管理员。建议拨打12345咨询。", "sources": []}
...
def ask_policy(question: str, lang: str = "zh", engine: str = "zhipu") -> dict:
    ...
    try:
        ...
        logger.warning("%s 调用失败: %s，尝试备选引擎...", engine, e)
        try:
            if engine == "zhipu":
                return ask_policy_deepseek(question, lang)
            else:
            ...
            return {
                "answer": "抱歉，AI服务暂时不可用，请稍后再试。建议直接拨打12345咨询。",
                "sources": [],
            }
